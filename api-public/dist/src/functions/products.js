...
async function listProducts(req, ctx) {
    ...
    const countryId = countryResult.recordset[0].Id;
    ...
    const langId = langResult.recordset[0]?.Id ?? null;
...
async function getProduct(req, ctx) {
    const id = parseInt(req.params.id);
    if (isNaN(id))
        return withCors({ status: 400, jsonBody: { error: "Invalid id" } });
    const languageCode = req.query.get("languageCode")?.toLowerCase() ?? "en";
    const pool = await getPool();
    const langResult = await pool
        .request()
...
async function optionsHandler(req, ctx) {
