...
async function queryAuditLog(req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> {
  ...
  if (tableName) dataRequest.input("TableName", sql.NVarChar(100), tableName);
  if (recordId) dataRequest.input("RecordId", sql.Int, parseInt(recordId));
  if (action) dataRequest.input("Action", sql.NVarChar(20), action);
  if (changedBy) dataRequest.input("ChangedBy", sql.NVarChar(36), changedBy);
  if (from) dataRequest.input("From", sql.DateTime2, new Date(from));
  if (to) dataRequest.input("To", sql.DateTime2, new Date(to));
