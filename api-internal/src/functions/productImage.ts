...
app.http("uploadProductImage", {
  ...
  handler: async (req: HttpRequest, _ctx: InvocationContext): Promise<HttpResponseInit> => {
    try {
      ...
      await writeAuditLog("Products", productId, "Update", user.entraObjectId,
        { imageBlobPath: null }, { imageBlobPath: blobPath, imageFileName: file.name });
...
app.http("deleteProductImage", {
  ...
  handler: async (req: HttpRequest, _ctx: InvocationContext): Promise<HttpResponseInit> => {
    try {
      ...
      await writeAuditLog("Products", productId, "Update", user.entraObjectId,
        { imageBlobPath: existing.recordset[0]?.ImageBlobPath }, { imageBlobPath: null });
...
function notFound(): HttpResponseInit { return { status: 404, jsonBody: { error: "Not found" } }; }
function forbidden(): HttpResponseInit { return { status: 403, jsonBody: { error: "Forbidden" } }; }
function serverError(err: any): HttpResponseInit {
  console.error(err);
