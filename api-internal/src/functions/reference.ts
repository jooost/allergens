...
async function listCategories(req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> {
  const user = await authenticate(req);
  if (!hasRole(user, "Reader")) return { status: 403, jsonBody: { error: "Forbidden" } };
...
async function listRegions(req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> {
  const user = await authenticate(req);
  if (!hasRole(user, "Reader")) return { status: 403, jsonBody: { error: "Forbidden" } };
...
async function listLanguages(req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> {
  const user = await authenticate(req);
  if (!hasRole(user, "Reader")) return { status: 403, jsonBody: { error: "Forbidden" } };
...
async function createCategory(req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> {
  const user = await authenticate(req);
  if (!user.roles.includes("Admin")) return { status: 403, jsonBody: { error: "Admin only" } };
...
async function updateCategory(req: HttpRequest, ctx: InvocationContext): Promise<HttpResponseInit> {
  const user = await authenticate(req);
  if (!user.roles.includes("Admin")) return { status: 403, jsonBody: { error: "Admin only" } };
