...
app.http("listProducts", {
  ...
  handler: async (req: HttpRequest, _ctx: InvocationContext): Promise<HttpResponseInit> => {
    try {
      ...
      const productIds = queryResult.recordset.map((r: any) => r.Id as number);

      // Batch-fetch suppliers for this page of products
      const supplierMap = new Map<number, { supplierId: number; name: string }[]>();
      if (productIds.length > 0) {
        const inList = productIds.map((_: number, i: number) => `@Pid${i}`).join(",");
        const supReq = pool.request();
        productIds.forEach((id: number, i: number) => supReq.input(`Pid${i}`, sql.Int, id));
        ...
        for (const r of supResult.recordset) {
          if (!supplierMap.has(r.ProductId)) supplierMap.set(r.ProductId, []);
          supplierMap.get(r.ProductId)!.push({ supplierId: r.SupplierId, name: r.SupplierName });
        }
      }

      return json({
        data: queryResult.recordset.map((row: any) => mapProductSummary(row, supplierMap)),
        page,
        pageSize,
        total,
...
function mapProductSummary(row: any, supplierMap?: Map<number, { supplierId: number; name: string }[]>) {
