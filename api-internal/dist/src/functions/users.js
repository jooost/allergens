...
async function grantPermission(req, ctx) {
    ...
    if (!body.regionId)
        return { status: 400, jsonBody: { error: "regionId is required" } };
    if (!body.role)
        return { status: 400, jsonBody: { error: "role is required" } };
...
async function revokePermission(req, ctx) {
    ...
    if (!existing.recordset[0])
        return { status: 404, jsonBody: { error: "Permission not found" } };
...
async function setPreferredLanguage(req, ctx) {
    const user = await authenticate(req);
    const body = (await req.json());
    if (!body.languageId)
        return { status: 400, jsonBody: { error: "languageId is required" } };
    const pool = await getPool();
    await pool
        .request()
        .input("EntraObjectId", sql.NVarChar(36), user.entraObjectId)
