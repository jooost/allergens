...
async function listDocuments(req, ctx) {
    const user = await authenticate(req);
    if (!hasRole(user, "Reader"))
        return { status: 403, jsonBody: { error: "Forbidden" } };
    const productId = parseInt(req.params.productId);
    if (isNaN(productId))
        return { status: 400, jsonBody: { error: "Invalid productId" } };
...
async function createDocument(req, ctx) {
    const user = await authenticate(req);
    ...
    if (!body.documentType)
        return { status: 400, jsonBody: { error: "documentType is required" } };
    if (!body.fileName)
    ...
    const sas = generateUploadSasToken(product.isoCode, productId, doc.Id, body.fileName, 1);
...
async function addDocumentVersion(req, ctx) {
    const user = await authenticate(req);
    if (!hasRole(user, "Editor"))
        return { status: 403, jsonBody: { error: "Forbidden" } };
    const productId = parseInt(req.params.productId);
    const documentId = parseInt(req.params.id);
    if (isNaN(productId) || isNaN(documentId))
    ...
    const nextVersion = maxVerResult.recordset[0].Next;
    const sas = generateUploadSasToken(product.isoCode, productId, documentId, body.fileName, nextVersion);
    const verResult = await pool
        .request()
        .input("DocumentId", sql.Int, documentId)
...
async function listDocumentVersions(req, ctx) {
    const user = await authenticate(req);
    if (!hasRole(user, "Reader"))
        return { status: 403, jsonBody: { error: "Forbidden" } };
    const productId = parseInt(req.params.productId);
...
async function uploadDocumentFile(req, _ctx) {
    try {
        const user = await authenticate(req);
        if (!hasRole(user, "Editor"))
            return { status: 403, jsonBody: { error: "Forbidden" } };
        ...
        if (!documentType)
            return { status: 400, jsonBody: { error: "documentType field is required" } };
        const validTypes = ["Specification", "AllergenDeclaration", "NutritionalCertificate", "LabelScan", "Other"];
