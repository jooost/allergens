...
CREATE TABLE Languages (
    Id          INT           NOT NULL IDENTITY(1,1),
    Code        VARCHAR(10)   NOT NULL,  -- e.g. 'en', 'fr', 'de'
    Name        NVARCHAR(100) NOT NULL,
...
CREATE TABLE AllergenIntensity (
    Id          INT           NOT NULL IDENTITY(1,1),
    Name        NVARCHAR(50)  NOT NULL,  -- 'Contains' | 'May Contain'
...
CREATE TABLE Allergens (
    Id          INT           NOT NULL IDENTITY(1,1),
    Name        NVARCHAR(100) NOT NULL,  -- canonical English name
...
CREATE TABLE UserProfiles (
    Id                  INT           NOT NULL IDENTITY(1,1),
    EntraObjectId       NVARCHAR(36)  NOT NULL,  -- Entra ID object GUID
...
CREATE TABLE UserPermissions (
    ...
    RegionId            INT           NOT NULL,
    CountryId           INT           NULL,       -- NULL = whole region
    Role                NVARCHAR(20)  NOT NULL,   -- 'Reader'|'Editor'|'Manager'|'Admin'
    GrantedBy           NVARCHAR(36)  NOT NULL,   -- Entra object ID of granting user
...
-- Full-text index for product search (requires full-text catalog)
...
CREATE FULLTEXT INDEX ON ProductTranslations (Name, Description)
    KEY INDEX UQ_ProductTranslations_ProductLanguage
    ON AllergenCatalog
...
CREATE TABLE ProductSuppliers (
    ...
    Priority    INT       NULL,       -- 1=primary, 2=first backup, etc. Optional.
...
-- Unique priority per product where priority is not null
...
CREATE TABLE ProductDocuments (
    Id               INT          NOT NULL IDENTITY(1,1),
    ProductId        INT          NOT NULL,
    FileType         NVARCHAR(20) NOT NULL,  -- 'PDF'|'Image'|'Certificate'|'Other'
    CurrentVersionId INT          NULL,       -- FK added after DocumentVersions created
    IsActive         BIT          NOT NULL CONSTRAINT DF_ProductDocuments_IsActive DEFAULT 1,
    UploadedBy       NVARCHAR(36) NOT NULL,
    UploadedAt       DATETIME2    NOT NULL CONSTRAINT DF_ProductDocuments_UploadedAt DEFAULT SYSUTCDATETIME(),
    ...
    CONSTRAINT CK_ProductDocuments_FileType CHECK (FileType IN ('PDF', 'Image', 'Certificate', 'Other'))
);

CREATE TABLE DocumentVersions (
    Id                  INT            NOT NULL IDENTITY(1,1),
    ProductDocumentId   INT            NOT NULL,
    VersionNumber       INT            NOT NULL,
    FileName            NVARCHAR(255)  NOT NULL,
    BlobStorageUrl      NVARCHAR(500)  NOT NULL,
    BlobContainerPath   NVARCHAR(500)  NOT NULL,
    ...
    CONSTRAINT UQ_DocumentVersions_DocVersion UNIQUE (ProductDocumentId, VersionNumber),
    CONSTRAINT FK_DocumentVersions_ProductDocuments FOREIGN KEY (ProductDocumentId) REFERENCES ProductDocuments (Id)
);
...
CREATE TABLE AuditLog (
    ...
    Action      NVARCHAR(20)   NOT NULL,  -- 'Insert'|'Update'|'Delete'|'StatusChange'|'Rollback'
